...
;
; 2007-04-19  edsa
;  Compatibility fix for Palette.
; 2006-04-18  edsa
;  BytesPerLine corrections. Support for Floodfill fix (see FILL.ASI)
...
PROC    Clear   NEAR
        ...
        mov     es, [VideoSeg]
        mov     ax, [PageOfs]
        mul     [BytesPerLine]          ; Startadresse des Bildschirms rechnen
...
; (X1/Y+1) zu landen
        ...
        mov     ax, [BytesPerLine]      ; ZeilenlÑnge in Pixeln
...
; Adresse, ds zeigt auf's Videosegment (keine Variablen mehr !!!)
        ...
        mov     ax, dx                  ; Y1
        add     ax, [PageOfs]
        mul     [BytesPerLine]         ; dx ist futsch
        add     ax, [SX1]
        adc     dl, 0                   ; öberlauf
...
; The following routine has been disabled for compatibility with the Borland
; EGAVGA driver and other third-party drivers.
;
; IF P80286
;        shr     bx, 2
;        shr     cx, 2
...
; aus, erfÅllt aber andere Kriterien ).
        ...
        mov     ax, [BytesPerLine]      ; ZeilenlÑnge in Bytes
IF P80286
        shl     ax, 3
ELSE
        mov     cl, 3
...
EVEN
StackBot        dw      ?               ; Unterer Teil des Stacks
