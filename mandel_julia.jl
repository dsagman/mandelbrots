...
function mandelbrot_set(width, height, rmin, rmax, imin, imax, max_iter)
    img = Matrix{RGB{Float64}}(undef, height, width)  # Initialize an image matrix with RGB values

    for y in 1:height
        for x in 1:width
            z = 0
            c = complex(rmin + (x - 1) / (width - 1) * (rmax - rmin), imin + (y - 1) / (height - 1) * (imax - imin))
            iteration = 0
            ...
            img[y, x] = calculate_color(iteration, max_iter)
        end
    end
